...
public class Node extends BaseTree<Node> implements TreeForAlphabeta{
    ...
    public Node(Node mother, int[][] stonePoints) {
        super(mother);
        this.stoneType = ENEMY_STONETYPE.ENEMY_STONETYPE(this.mother.stoneType);
...
//    }
    ...
    private int[][] updateBoard(int[][] board, int[][] stonePoints) {
        ...
        for (int[] stonePoint:
             stonePoints) {
            result[stonePoint[X]][stonePoint[Y]] = ENEMY_STONETYPE.ENEMY_STONETYPE(this.stoneType);
        }
        return result;
    }
    ...
    public void extend() {
        if(!this.extended) {
            for (int[][] stonePointPair :
                    PossibleNextStone.possibleNextStonePair(this.getBoard())) {
...
//        boardCompare(getBoard());
    ...
    public double getEval() {
        if (this.eval == null) {
            this.eval = Evaluation.evaluation(getBoard(), ENEMY_STONETYPE.ENEMY_STONETYPE(getRoot().getStoneType()));
